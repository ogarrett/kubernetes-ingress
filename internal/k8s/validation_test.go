...
func TestValidateNginxIngressAnnotations(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			internalRoutesEnabled: false,
			expectedErrors: []string{
				fmt.Sprintf(`annotations.appprotectdos.f5.com/app-protect-dos-security-log-destination: Invalid value: "Not Valid": Error parsing App Protect Log config: Destination must follow format: <ip-address | localhost | dns name>:<port> or stderr Log Destination did not follow format`),
			},
